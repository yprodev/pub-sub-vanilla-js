...
export const stats = {
  render: container => {
    ...
    pa.innerHTML = '0 actors in a list';
    d.appendChild(pa);
  ...
  moviesUpdated: list => {
    ...
    console.log(`STATS: I hear that the movie list now has ${list.length} titles.`);
    document.querySelector('.movie-count').innerText = `${list.length} movies in list`;
  },
  actorsUpdated: list => {
    // the list of movies was just published as updated
    console.log(`STATS: I hear that the actor list now has ${list.length} names.`);
    document.querySelector('.actor-count').innerText = `${list.length} actors in list`;
  },
};
